...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 设置水平滚动条隐藏
        _XHScrollView.showsHorizontalScrollIndicator = NO;
...
- (void)initCard:(NSArray *)cardArr {
    ...
    for (UIView *cardView in cardArr) {
        ...
        if (index == 0) {
            viewWidth = CGRectGetWidth(cardView.bounds);
        }
        space = CGRectGetWidth(self.frame) - viewWidth;
        width = space / 4 + viewWidth;
        cardView.frame = CGRectMake(space / 2 + (CGRectGetWidth(cardView.frame) + space / 4) * index, 0, viewWidth, cardView.frame.size.height);
        //形变
    ...
    _XHScrollView.contentSize = CGSizeMake((space / 2 + width * cardArr.count) + space / 4, CGRectGetHeight(_XHScrollView.frame));
}
